...
class AuthRepository(private var auth: FirebaseAuth,  private var apiService: ApiService) {
    ...
    fun register(email: RequestBody, password: RequestBody, username: RequestBody): LiveData<Result<RegisterResponse>> = liveData {
        emit(Result.Loading)
        try {
    ...
    fun login(email: RequestBody, password: RequestBody): LiveData<Result<LoginResponse>> = liveData {
        emit(Result.Loading)
        try {
